$ErrorActionPreference = 'Stop'
$source = Get-Content -Raw (Join-Path $PSScriptRoot '..\Core\Src\main.c')

$resetStart = $source.IndexOf('static void ADS_HardwareReset(void)')
$resetEnd = $source.IndexOf('static uint8_t ADS_CommandModeProbe(void)', $resetStart)
if (($resetStart -lt 0) -or ($resetEnd -lt 0)) { throw 'Could not isolate ADS hardware reset' }
$reset = $source.Substring($resetStart, $resetEnd - $resetStart)

$sequence = @(
    'HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4, GPIO_PIN_SET)',
    'HAL_Delay(10U)',
    'HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4, GPIO_PIN_RESET)',
    'HAL_Delay(100U)',
    'HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4, GPIO_PIN_SET)',
    'HAL_Delay(200U)'
)
$cursor = 0
foreach ($item in $sequence) {
    $position = $reset.IndexOf($item, $cursor)
    if ($position -lt 0) { throw "Missing/ordered ADS power-up step: $item" }
    $cursor = $position + $item.Length
}

$start = $source.IndexOf('static HAL_StatusTypeDef ADS_ConfigureAndStartAttempt(void)')
$end = $source.IndexOf('static HAL_StatusTypeDef ADS_StartAcquisition(void)', $start)
if (($start -lt 0) -or ($end -lt 0)) { throw 'Could not isolate ADS acquisition startup' }
$init = $source.Substring($start, $end - $start)
if (($init.IndexOf('ADS_HardwareReset()') -lt 0) -or
    ($init.IndexOf('ADS_EnterCommandMode()') -lt 0)) {
    throw 'Each acquisition attempt must power/reset the ADS and enter verified command mode.'
}

if (($source -notmatch 'ads_last_failed_id = ads_id_value') -or
    ($source -notmatch 'ADS recovery: ID=%lu') -or
    ($source -notmatch 'ads_recovery_reason')) {
    throw 'Recording transition must expose the failed ADS ID through recovery diagnostics.'
}

Write-Output 'ADS power-up after boot upload contract: PASS'
